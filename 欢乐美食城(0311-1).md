...
近期因为疫情在家办公，再工作之余，继续看了下书，以及学习视频，发现效率很低，想着，要不就刷题吧，起码应对面试，也能一套一套的，但是没有项目支撑的知识结构，总让自己的心里觉得很虚。所以还是想刷题的同时，自己从脚手架搭建，到项目完成，走一套完成的前后端流程，从而练习一下node,与koa2、typescript。关于项目选择怎样的项目，由于自己没美食比较有兴趣，之前关注了深证美食俱乐部的公众号，看到上面的美食推荐，一直有个想法，就是把他们统计起来，然后根据评价地址，一一对他们翻牌。目前想做的功能是，对广东省内的美食进行分区统计，并自己给出一套推荐指数算法。之后还想对广东省的美食，以及旅游地点进行统计。其实会python的话，应该可以直接爬取数据，但我现在想巩固node的功能，所以想采取录入的方式，之后想学python了，再考虑爬取数据的方式
-20200311
...
我对express,和koa1,koa2,了解得都不深，一直以来，我都是一个很“纯粹”的前端，看了网上的,koa2与koa1的最大区别是koa2实现异步是通过async/awaite，koa1实现异步是通过generator/yield，而express实现异步是通过回调函数的方式。koa更加的干净简洁，所以，决定先用koa2吧。

##### 前端react
react和vue打架了很久，两者其实做项目，都会做，但是vue比react先上手，那时候只想着能做好业务就行了，没有太纠结原理，所以，对vue的了解大概就是结合element-ui写项目，后面换了公司，用的react，开始知道，对框架应该有更深层次的理解，学了redux之类的东西，对react的写法也越来越喜欢，所以，决定一个一个框架深究着来，不要一下这个一下那个的，所以选了react，希望通过做这个项目，可以对react做项目有个更深入的了解

### 脚手架
其实每个框架已经有了现成的脚手架工具了，但还是想自己打架脚手架，从而对webpack更加的了解，之前搭建的脚手架已经支持react和typescript,还缺路由与koa2,今天想先把之前的脚手架补充完整，再考虑项目的具体功能与实现。
